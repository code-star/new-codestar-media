...
function dot(g, x, y, w, options) {
    options = { fill: b, accent: false, round: false, ...options };
    ...
    if (!round) {
        ...
        g
            ...
            .fill(accent ? c : fill)
            .stroke({ color: accent ? c : b, width: fill === "transparent" ? stroke: 0 })
            .center(w * x, w * y);
    }
}
