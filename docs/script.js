...
const colorParam = f1.addBinding(params, "color", {
  label: "Star color",
  options: color_names,
});
...
const templateParam = f3.addBinding(params, "template", {
  label: "Template JSON",
});

const applyTemplateButton = f3.addButton({
...
function asteriskFront(x = 9.7, y = -7.6) {
  const g = drawFront.group();
  const factor = 4.333;
  const n = 2 / factor;
  const m = 8 / factor;
  g.rect(n, m).fill(c()).center(x, y).rotate(90);
  g.rect(n, m).fill(c()).center(x, y).rotate(30);
  g.rect(n, m).fill(c()).center(x, y).rotate(-30);
  return g;
}
...
function codestar() {
  ...
  const { beams, circles } =
    params.star_type < 0 ? setups[random2] : setups[params.star_type];
  ...
  const color_name = Object.keys(color_names).find(
    (k) => color_names[k] === color_index
  );

  const star_name = Object.keys(star_names).find(
    (k) => star_names[k] === (params.star_type < 0 ? random2 : params.star_type)
  );

  const text1 = `${color_name.toLowerCase()}-${star_name
    .toLowerCase()
    .replace(" ", "-")}`;

  const text2 = `i${Math.floor(params.ion_probability * 100)}-d${Math.floor(
    params.particle_density * 100
  )}-a${Math.floor(params.arc_probability * 100)}`;
